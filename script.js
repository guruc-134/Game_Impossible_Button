...
gameBtn.addEventListener('click', function()
{
    ...
    {
        container.innerHTML="<h2 class='neutral_txt'> <span class='giveup_span' >Its' ok to Giveup</span> Start/Reset the game</h2>"
        container.classList.add('neutral');
...
function game()
{
    ...
    containerWidth=parseInt(styles.width,10);
    containerHeight=parseInt(styles.height,10);
    let setwidth=Math.max(0 , Math.floor(containerWidth*Math.random()-30));
    let setheight=Math.max(0, Math.floor(containerHeight*Math.random()-30));
    // console.log(setheight,setwidth);
...
>>>>>>> d8a6e853b192a70a264c0f53c8d49f0adaff8f52
